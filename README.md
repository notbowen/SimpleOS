...
**Fix:**
Turns out, I compiled the programs in 64 bits in an attempt to "fix" a linker error, which stated that the i386 format was not compatible with x86_64. This was resolved by compiling the programs in 32 bits, then linking with the option `-m elf_i386`, and compiling C sources with the argument `-fno-pie`.
